...
7. **Utilities** (`src/utils/`):
   ...
   - `nqlSanitizer.js`: NQL query sanitization — exports `sanitizeNqlValue`, the canonical helper for escaping values in NQL filter strings (escapes backslashes, single quotes, and double quotes). Used by memberService, tierService, and the MCP server's tag filter builder (`ghost_get_tags`). All NQL value escaping must go through this function; do not add local escape helpers elsewhere.
